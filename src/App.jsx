  ...
  return (
    <ul>
      {friends.map((friend, i) => (
        <Friend key={i} friend={friend} onSelect={onSelection} selectedFriend={selectedFriend} />
      ))}
    </ul>
  );
}
  ...
  return (
    <form className="form-split-bill" onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        value={paidByUser}
        onChange={(e) => setPaidByUser(Number(e.target.value) - bill > 0 ? paidByUser : Number(e.target.value))}
      />

      <label>👯‍♂️ Friend's expense</label>
      <input type="text" disabled value={paidByFriend} />

      <label>😑 Who is the bill ?</label>
